...
export const companySchema = yup.object({
  name: yup.string().required('Company name is required').max(20, 'Max 20 characters allowed'),
...
//   .matches(/^[a-z0-9-]+$/, 'Lowercase alphanumeric and hyphen only')
  tenant_code: yup.string().required('Tenant Code is required').max(6, 'Max 6 characters allowed'),
